          ...
          <div className="px-4">
            ...
            {entries.map((e) => {
              ...
              const mainLabel = isHK
                ? (isZh(lang) ? t(stk?.nameCN || STOCK_CN_NAMES[e.ticker] || stk?.name || e.name) : (stk?.name || e.name)) || e.ticker
                : e.ticker;
                ...
                {addSearchResults.map(r => (
                  ...
                  >
                    ...
                    <span className="truncate flex-1 text-[11px]" style={{ color: "var(--fg-2)" }}>
                      {isZh(lang) ? t(STOCK_CN_NAMES[r.symbol] || r.name) : r.name}
                    </span>
                  </button>
                ))}
              </div>
      ...
      <div className={`md:col-span-4 flex flex-col gap-3 md:min-h-0 ${mobileShowDetail ? "hidden md:flex" : "flex"}`}>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 glass-card z-20 max-h-[200px] overflow-auto" style={{ boxShadow: "var(--bg-card-shadow)" }}>
                  {addSearchResults.map(r => (
                    ...
                    >
                      ...
                      <span className="truncate text-[10px] flex-1" style={{ color: "var(--text-secondary)" }}>{isZh(lang) ? t(STOCK_CN_NAMES[r.symbol] || r.name) : r.name}</span>
        ...
        <div className="flex-1 overflow-auto space-y-2">
          ...
          {entries.filter(e => journalTagFilter === "all" || (e.tags || []).includes(journalTagFilter)).map(e => {
            ...
            const isHK = e.ticker?.endsWith(".HK");
            const mainLabel = isHK
              ? (isZh(lang) ? t(stk?.nameCN || STOCK_CN_NAMES[e.ticker] || stk?.name || e.name) : (stk?.name || e.name || STOCK_CN_NAMES[e.ticker])) || e.ticker
              : e.ticker;
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium" style={{ color: "var(--text-heading)" }}>
                      {t('投资论点')} — {sel.ticker?.endsWith(".HK")
                        ...
                        : <>{sel.ticker} {isZh(lang) ? t(STOCK_CN_NAMES[sel.ticker] || sel.name) : sel.name}</>}
                    </span>
                  </div>
