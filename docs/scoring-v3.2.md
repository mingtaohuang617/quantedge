...
- 将评分改造移植至生产主分支 2ed8b75，保留认证 BFF、行情分片及每日研究功能。
- 最新 React 19 / Vite 8 环境：757 项前端单元测试、生产构建、包体积、多语言和发布门禁通过。
- 评分页桌面及手机浏览器回归 2 项通过；后端 Ruff 检查通过。
- 市场分片由本次评分快照重新生成，避免加载旧评分数据。
- 通过拉取请求 CI 和预览验证后合并，生产部署继续使用现有自动校验与回滚流程。
