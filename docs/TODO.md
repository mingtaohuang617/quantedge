...
> 2026-08-28 说明：本文档主体是 2026-05 至 2026-06 的实施档案，不再作为当前发布清单。未实施的 vectorbt、HMM 扩展、TimescaleDB、ETF Universe Manager 与外部通知统一转入 [未来研究基础设施 RFC](rfcs/FUTURE_RESEARCH_INFRASTRUCTURE.md)，本轮不执行。
...
- 本文档中的完成记录保留为历史证据，旧的优先级数量和“另一会话 stash”不再用于判断当前状态。
- Smart Beta FastAPI 路由集成测试已于 2026-05-24 完成，不再列为可选 TODO。
- 外部数据库、通知渠道、动态 ETF universe 和新回测引擎必须先通过 RFC 触发条件，再建立新的、可验收的实施清单。

## 近期大块已落地（2026-06，本文件 P0-P3 之外的主线）

> 详见 [CHANGELOG.md](../CHANGELOG.md) [Unreleased]

- **评分机制双轨重构** ✅ 全交付：P1 数据地基(A股/港股基本面回补 + GICS 归一, #241) → P2 双轨引擎 + P3 前端(#243) → 基本面钳制(#245) → 策略回测(#248)。**根治「74% A股/港股无基本面→综合分≈纯动量」病根**(现 PE 覆盖 91-94%)。后续体检发现并修了「时机分离散度≈2×质量、令 0.6 质量权重名不副实」→ 双轨等方差标准化(PR #250)。详见 memory `module_scoring_overhaul`。
- **K线浏览平台 + TradingView 化** ✅：点击放大成 K线平台(#232/#234) → 常驻图例/最新价线/对数轴/十字光标/滚轮缩放(#239) → 画线工具 趋势线/水平线/测量 + 指标自定义周期(#244) → 画线持久化(#247) → 移动端 K线(PR #249)。
- 评分页桌面三栏工作站(#230) + 对比盘空态折叠(#238)。
