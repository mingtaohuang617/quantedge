...
→ **图表线/评分线均已成熟收尾**。可挑的 S 级小活：
1. **水平线跨周期复用** — 画线持久化现按 (标的×周期) 分桶；水平价格警戒线(仅 price 锚)可改为按标的跨周期显示。
2. **刷新本 TODO 的 P0-P3 历史区** — 多处停在 2026-05-21，剩余项快查表已过时。
3. Smart Beta 后续(BacktestEngine 接入 / ETF 筛选器)或 vectorbt 向量化回测(见下方剩余项)。
