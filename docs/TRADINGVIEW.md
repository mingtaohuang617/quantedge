...
当前代码仅提供 1D 行情和同周期指标，不自动覆盖财报、评分或回测输入。[PR #335](https://github.com/mingtaohuang617/quantedge/pull/335) 增加评分页行情展示叠加和云端每日刷新；实现、时间和边界见文末。是否已部署及定时任务是否成功，以 CI / Daily market refresh 实际运行记录为准，不把代码存在视为线上验收。
