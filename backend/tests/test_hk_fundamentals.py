...
验证 P1 港股财务补充源已正确接入 pipeline:
  yfinance 字段 None → AKShare 补齐 → 仍 None 才回落 static_overrides
"""
...
class TestApplyHkFundamentalsFallback:
    def test_non_hk_is_noop(self):
        """美股标的应跳过 AKShare 调用，原 result 不变。"""
    ...
    def test_hk_fills_only_none_fields(self):
        """HK 标的 + yfinance 全 None → AKShare 4 字段都被填。"""
        result = {"pe": None, "roe": None, "revenueGrowth": None, "profitMargin": None}
        ak_data = {
    ...
    def test_does_not_overwrite_existing_values(self):
        ...
        assert out["roe"] == 11.2                # 补 AKShare
        assert out["revenueGrowth"] == 12.0      # 保留 yfinance
        assert out["profitMargin"] == 28.0       # 补 AKShare

    def test_akshare_none_value_does_not_overwrite_none(self):
        """AKShare 字段也是 None → result 字段保持 None。"""
        result = {"pe": None, "roe": None, "revenueGrowth": None, "profitMargin": None}
    ...
    def test_akshare_exception_silently_returns(self, capsys):
        """AKShare 抛错应被吞掉 + 日志，不污染 pipeline 主流程。"""
        result = {"pe": None, "roe": None, "revenueGrowth": None, "profitMargin": None}
        with patch.object(pipeline, "fetch_hk_fundamentals",
...
class TestFallbackOrder:
    def test_static_overrides_only_fills_what_akshare_left_none(self):
        """链路：yfinance(None) → AKShare(部分 None) → static_overrides(兜余下)。"""
        # 模拟：yfinance 给出 None，AKShare 只给 pe 和 profitMargin，
        ...
        # AKShare 已填的不被静态值覆盖（仅 None 才补）
        assert result["pe"] == 8.0           # AKShare 优先于 static
        assert result["profitMargin"] == 25.0
        # AKShare 没填的，static_overrides 补
        assert result["roe"] == 15.0
        assert result["revenueGrowth"] == 5.0


# ── 真实网络拉数据（已知失败：akshare_source 字段映射问题，留待修）─
...
def test_real_0005hk_fundamentals():
    """真实拉 0005.HK 验证至少能拿到 pe 或 market_cap（AKShare 在线 sanity check）。"""
    data, src = pipeline.fetch_hk_fundamentals(HK_CFG)
    assert isinstance(data, dict)
    # 至少一个数值字段非空
    has_any = any(data.get(k) is not None for k in
                  ("pe", "roe", "market_cap", "profit_margin"))
    assert has_any, f"AKShare 港股完全无数据: src={src}, data={data}"
