...
# ─── 多时间维度价格历史 ───────────────────────────────────
# 各时间维度的 yfinance 参数及采样点数
PRICE_RANGES = {
    "1D":  {"period": "1d",  "interval": "5m",  "points": 48,  "fmt": "%H:%M"},
    "5D":  {"period": "5d",  "interval": "15m", "points": 40,  "fmt": "%m/%d %H:%M"},
    "1M":  {"period": "1mo", "interval": "1h",  "points": 30,  "fmt": "%m/%d"},
    "6M":  {"period": "6mo", "interval": "1d",  "points": 60,  "fmt": "%m/%d"},
    "YTD": {"period": "ytd", "interval": "1d",  "points": 60,  "fmt": "%m/%d"},
    "1Y":  {"period": "1y",  "interval": "1d",  "points": 60,  "fmt": "%y/%m/%d"},
    "5Y":  {"period": "5y",  "interval": "1wk", "points": 60,  "fmt": "%y/%m"},
    "ALL": {"period": "max", "interval": "1mo", "points": 60,  "fmt": "%Y/%m"},
}


def fetch_price_ranges(tk, ticker_key: str) -> dict:
    """
    拉取多个时间维度的价格历史。
    返回 { "1D": [...], "5D": [...], ... }
    每项为 [{"m": dateLabel, "p": price}, ...]
    """
    result = {}
    for range_key, params in PRICE_RANGES.items():
        try:
            hist = tk.history(
            ...
            if hist.empty or len(hist) < 2:
                log(f"    ⚠ {ticker_key} {range_key}: 无数据，跳过")
                continue

            close = hist["Close"]
            n = len(close)
            max_pts = params["points"]
            indices = np.linspace(0, n - 1, min(max_pts, n), dtype=int)

            pts = []
            for idx in indices:
                row = hist.iloc[idx]
                date_str = row.name.strftime(params["fmt"])
                pts.append({"m": date_str, "p": round(float(row["Close"]), 2)})
            result[range_key] = pts
        except Exception as e:
            log(f"    ⚠ {ticker_key} {range_key}: 获取失败 ({e})")
    return result
...
def fetch_stock_data(ticker_key: str, cfg: dict) -> dict | None:
    ...
    try:
        ...
        if hist.empty:
            log(f"  ⚠ {ticker_key}: 无法获取行情数据，尝试备用周期...")
            hist = tk.history(period="1mo")
            if hist.empty:
                log(f"  ✗ {ticker_key}: 行情数据完全不可用")
                return None
        ...
        # 综合评分
        score, sub_scores = calc_stock_score(pe, roe, revenue_growth, profit_margin, momentum, rsi)

        # 构建价格历史 (用于前端图表) — 保留默认的6M数据作为 priceHistory
        price_history = []
        ...
        # 多时间维度价格数据
        price_ranges = fetch_price_ranges(tk, ticker_key)
...
def fetch_etf_data(ticker_key: str, cfg: dict) -> dict | None:
    ...
    try:
        ...
        result = {
            ...
            "trackingError": "较高 (杠杆损耗)" if cfg.get("leverage") else None,
            "aum": aum_str,
